...
export default function MarketplaceByIdClient({ id }: Props) {
  ...
  // - si viene "VCS-844" => mandamos "844"
  // - si viene "844" => mandamos "844"
  // - si viene otro registry "GS-123" => mandamos "123" (por si acaso)
  const normalizedId = (() => {
    if (typeof id !== 'string') return '';
    const m = id.match(/^\w+-([0-9]+)$/); // "VCS-844" => ["VCS-844","844"]
    if (m?.[1]) return m[1];
    return id;
  })();

  const { project, handleRetire, prices, isPricesLoading } = useMarketplace(normalizedId);

  // ✅ Evitamos el loader infinito: si ya dejó de cargar precios y project sigue null,
  // mostramos un mensaje claro.
  if (!project && !isPricesLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center p-6">
        <div className="max-w-xl text-center">
          <p className="text-xl font-medium text-gray-700">Proyecto no encontrado</p>
          <p className="mt-2 text-gray-500">
            ID recibido: <span className="font-mono">{id}</span> — Normalizado:{' '}
